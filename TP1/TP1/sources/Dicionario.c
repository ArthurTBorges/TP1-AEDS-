...
void ImprimeDicio(TListaLetra* pLetra){
    ...
    while(pAuxL != NULL){
        ...
        while (pAuxL2 != NULL){
            ...
            fprintf(in_file, "palavra: %s", pAuxL->primLtr->proxLetra->letter.ListaDaLetra.primP->proxPalavra->Item.string);

            while (pAuxL3 != NULL){
                fprintf(in_file, "palavra: %d", pAuxL->primLtr->proxLetra->letter.ListaDaLetra.primP->proxPalavra->Item.primL->proxLinha->line.numero);
